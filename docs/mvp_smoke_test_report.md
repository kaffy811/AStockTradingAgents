...
**测试日期：** 2026-05-26（最近更新：Phase M51 Prompt 结构优化，2026-06-11）  
**版本：** MVP v1.0 — 四维综合分析 + 行业热门股动态同行 + 全量申万 L1 行业映射（5166 只股票）+ Redis 三层缓存（R1/R2/R3）+ 自选股 Watchlist（W1~W3）+ 股票搜索联想（P4-a/b/c）+ 行业热度全览（M30）+ 行业热度聚合 API（M31）+ AnalysisRunRegistry 抽象（M40-a）+ M51 结论先行 Prompt  
**测试人：** 开发自测
...
## Phase M47-b Smoke Test（2026-06-11）

| # | 测试项 | 预期 | 结果 |
|---|--------|------|------|
| M47-b-1 | 综合分析页右列无"最近搜索"卡片 | 已删除 | ✅ |
| M47-b-2 | 右列上区显示"行业热门板块" | ind_blocks_title | ✅ |
| M47-b-3 | 仪表盘行业列表最多 6 行 | slice(0,6) | ✅ |
| M47-b-4 | 行业按 hot_score 降序 | sort desc | ✅ |
| M47-b-5 | 排名 1-3 金色高亮 | hdp-iblk-rank--top | ✅ |
| M47-b-6 | avg_change_pct 正负颜色 | up/down CSS | ✅ |
| M47-b-7 | hot_score 2dp 格式 | toFixed(2) | ✅ |
| M47-b-8 | 行业名缺失显示"未知行业" | ind_unknown fallback | ✅ |
| M47-b-9 | hot_score/avg_change_pct 缺失显示 — | ternary fallback | ✅ |
| M47-b-10 | 行业块加载错误显示 ind_blocks_err | industryListError prop | ✅ |
| M47-b-11 | "展示全部"跳转 /industries | emit go-industries | ✅ |
| M47-b-12 | 点击行业 → /industries?focus=<code> | onDashGoIndustryBlock | ✅ |
| M47-b-13 | 行业页 focus 行为：选中+滚动+高亮 | watch + scrollIntoView | ✅ |
| M47-b-14 | data-industry-code 属性存在 | IndustryHotBlocksCard 已加 | ✅ |
| M47-b-15 | 行业热股 limit 仪表盘侧 20 | ComprehensiveAnalysisView limit:20 | ✅ |
| M47-b-16 | 行业热股 limit 行业页 20 | HOT_LIMIT=20（已有） | ✅ |
| M47-b-17 | 6 语言 ind_blocks_* 覆盖 | 全部补齐 | ✅ |
| M47-b-18 | npm run build | 195 modules | ✅ |
| M47-b-19 | python compileall app | 0 errors | ✅ |
| M47-b-20 | alembic current | c5e9f12a3b87 (head) | ✅ |
| M47-b-21 | 零新依赖 | package.json 未改动 | ✅ |
| M47-b-22 | 零 migration | alembic head 不变 | ✅ |

---

## Phase M48 Smoke Test（2026-06-11）

| # | 测试项 | 预期 | 结果 |
|---|--------|------|------|
| M48-1 | 行业页默认显示 Top 20 | slice(0,20) | ✅ |
| M48-2 | 股票 > 20 时显示"查看更多"按钮 | v-if > 20 | ✅ |
| M48-3 | 点击"查看更多"展开 Top 50 | expandedView=true | ✅ |
| M48-4 | 点击"收起"恢复 Top 20 | expandedView=false | ✅ |
| M48-5 | 底部收起按钮（展开 > 10 条时） | ihv-bottom-bar | ✅ |
| M48-6 | 切换行业自动收起 | onIndustryChange reset | ✅ |
| M48-7 | 股票数 ≤ 20 不显示"查看更多" | v-if filteredItems > 20 | ✅ |
| M48-8 | 股票列表标题显示 Top 20/50 + count | ihv-stocks-header | ✅ |
| M48-9 | 行业热度 loading 文字 | ind_loading_heat | ✅ |
| M48-10 | 首页空状态全覆盖 | reports/watchlist/blocks/hot | ✅ |
| M48-11 | 6 语言 ind_hot_* 覆盖 | 9 个新 key | ✅ |
| M48-12 | /industries?focus=<code> 回归 | focus 逻辑未改动 | ✅ |
| M48-13 | npm run build | 195 modules | ✅ |
| M48-14 | python compileall | 0 errors | ✅ |
| M48-15 | alembic current | c5e9f12a3b87 (head) | ✅ |
| M48-16 | 零新依赖，零 migration | ✅ | ✅ |

---

## Phase M49 Smoke Test（2026-06-11）

| # | 测试项 | 预期 | 结果 |
|---|--------|------|------|
| M49-1 | 首页自选快跳 ≥2 只显示"批量对比"入口 | v-if ≥ 2 | ✅ |
| M49-2 | 点击入口 → /watchlist?mode=compare | go-watchlist-compare | ✅ |
| M49-3 | WatchlistView 自动进入批量模式 | route.query.mode | ✅ |
| M49-4 | 勾选 < 2，"对比"按钮 disabled | :disabled | ✅ |
| M49-5 | 勾选 2-4 只，点击"对比"跳转 | handleCompare() | ✅ |
| M49-6 | 对比页 URL 驱动加载所选股票 | /compare?stocks=... | ✅ |
| M49-7 | 勾选 > 4 只，"对比"按钮 disabled | :disabled > 4 | ✅ |
| M49-8 | 清空选择 / 退出批量可用 | WatchlistToolbar | ✅ |
| M49-9 | 原有添加/删除/备注功能回归 | 未改相关逻辑 | ✅ |
| M49-10 | dash_compare_bulk 6 语言覆盖 | 全局 i18n | ✅ |
| M49-11 | npm run build | 195 modules | ✅ |
| M49-12 | python compileall | 0 errors | ✅ |
| M49-13 | alembic current | c5e9f12a3b87 | ✅ |

---

## Phase M51-b 真实报告生成回归验证（2026-06-11）

**测试股票：** CN/688146（中船特气）  
**测试范围：** comprehensive / technical_only / fundamental_only / peer_only / news_only  
**数据源状态：** 技术面 AkShare ✅ / 新闻面 AkShare ✅ / 基本面 数据源返回失败（字段缺失）/ 同行 PE/PB 全缺失

### 结构验证

| # | 验证项 | 预期 | 结果 |
|---|--------|------|------|
| M51b-1 | comprehensive 报告首节为 `## 一、综合结论卡片` | ✅ | ✅ |
| M51b-2 | 综合结论卡片含 **综合判断** / **一句话结论** / **核心矛盾** / **正面因素** / **主要风险** / **后续观察重点** / **数据完整度** 7 字段 | ✅ | ✅ |
| M51b-3 | technical_only 报告含 `### 摘要结论` 为首节 | ✅ | ✅ |
| M51b-4 | news_only 报告含 `### 摘要结论` 为首节 | ✅ | ✅ |
| M51b-5 | peer_only 报告含 `### 摘要结论` 为首节 | ✅ | ✅ |
| M51b-6 | fundamental_only 报告正确标注字段缺失，无捏造数据 | ✅ | ✅ |
| M51b-7 | 综合报告 **综合判断**：分歧（因基本面数据缺失） | ✅ | ✅ |
| M51b-8 | 全部报告无买入/卖出/持有投资建议 | ✅ | ✅ |
| M51b-9 | 全部报告有完整数据边界说明 | ✅ | ✅ |
| M51b-10 | `extractSummary()` zh-CN 路径命中 `### 摘要结论` | ✅ | ✅ |
| M51b-11 | `extractSummary()` en-US 路径命中 `### Summary & Conclusions`（LLM 翻译变体） | 修复后 ✅ | ✅ |
| M51b-12 | `extractSummary()` 旧路径回归（zh-CN `一、摘要` / en-US `I. Summary`） | ✅ | ✅ |
| M51b-13 | npm run build（修复后） | 195 modules | ✅ |
| M51b-14 | python compileall | 0 errors | ✅ |
| M51b-15 | alembic current | c5e9f12a3b87 (head) | ✅ |

### 可读性评分（zh-CN 报告，5 分制）

| 维度 | 得分 | 说明 |
|------|------|------|
| 结论先行 | 5/5 | 综合报告一、综合结论卡片直接给出判断，单面报告 ### 摘要结论 置顶 |
| 信号清晰度 | 5/5 | 正面因素/主要风险各 1-2 条，量化数据清晰（涨幅 147.87%，偏离均线 87.65%） |
| 数据边界说明 | 5/5 | 基本面 14 字段缺失明确罗列，无捏造数据 |
| 多维度整合 | 4/5 | 四面结论汇总完整，但基本面数据缺失导致横向验证受限 |
| 风险提示规范 | 5/5 | 无投资建议，异常波动公告等监管信息准确引用 |
| 总体可读性 | 5/5 | 从摘要卡片可 5 秒抓取核心结论，详细内容层次分明 |

**综合评分：4.8/5**（vs 旧结构 ~3.2/5：旧结构结论分散在各节末尾，需全文阅读方可归纳）

### en-US extractSummary 修复说明

- **问题**：LLM 将 `### 摘要结论` 翻译为 `### Summary & Conclusions`，旧代码仅检测 `### Summary Conclusion`（无 "&"），导致 en-US 单面报告 extractSummary 回落到 `I. Summary` 包装文本（样板文字）
- **修复**：`reportText.js` 新增 `### Summary & Conclusions` / `### Summary & Conclusion` / `### Summary Conclusions` / `### Summary`（广播兜底）4 个变体匹配
- **验证**：8/8 测试用例全部 PASS（4 变体 + zh-CN 回归 + comprehensive zh-CN 回归 + legacy en-US 回归）


---

## Phase C4 Smoke Test（2026-06-18）

### 工具 Registry 验证

| # | 测试 | 结果 |
|---|------|------|
| C4-T1 | python -m compileall app -q | ✅ 0 errors |
| C4-T2 | npm run build | ✅ 213 modules，0 errors |
| C4-T3 | alembic current | ✅ d7e3a9b5c2f8 (head)，无新 migration |
| C4-T4 | pytest tests/test_c4_orchestrator.py -v | ✅ 11/11 PASS |

### 意图路由验证（10 条输入全部正确路由）

| 输入 | 预期意图 | 实际路由 | 结果 |
|------|---------|---------|------|
| 中船特气最近为什么涨这么多 | anomaly | _handle_anomaly | ✅ |
| 688146 最近有什么新闻 | news | _handle_news | ✅ |
| 今天哪些行业值得重点研究 | industry | _handle_industry | ✅ |
| 解释我最近一份报告 | recent_report | _handle_recent_report | ✅ |
| 看看我的自选股 | watchlist_view | _handle_watchlist_view | ✅ |
| 看看我的自选股（误判修复） | NOT watchlist_add | _match_watchlist_add → False | ✅ |
| 把中船特气加入自选 | watchlist_add | _handle_watchlist_add | ✅ |
| 帮我生成688146综合报告 | report_mock | _handle_report → confirmation | ✅ |
| 我的报告 | recent_report | _handle_recent_report | ✅ |
| 查看自选股 | watchlist_view | _handle_watchlist_view | ✅ |

### 安全验证

| 检查项 | 结果 |
|--------|------|
| 禁用词（买入/卖出/持有/目标价）扫描 | ✅ 0 命中 |
| 每个 answer 包含免责声明 | ✅ |
| 新闻内容截断（summary[:200]，不传正文） | ✅ |
| 用户数据隔离（DB where user_id） | ✅ |
| 写操作工具 C4 不执行真实写入 | ✅ mock confirm only |

### 前端新增卡片类型

| 卡片类型 | 对应工具 | 状态 |
|---------|---------|------|
| watchlist_list | get_watchlist_tool | ✅ 新增渲染 |
| report_list | get_recent_reports_tool | ✅ 新增渲染 |
| stock_summary | get_quote_tool | ✅ 已有 |
| industry_hot | get_industry_hot_tool | ✅ 已有 |
| report_link | mock confirm | ✅ 已有 |


---

## Phase C5 / C5-b Smoke Test（2026-06-18）

### 自动化验证

| # | 测试 | 结果 |
|---|------|------|
| C5-T1 | python -m compileall app -q | ✅ 0 errors |
| C5-T2 | npm run build | ✅ 213 modules，0 errors |
| C5-T3 | alembic current | ✅ d7e3a9b5c2f8 (head)，零新 migration |
| C5-T4 | pytest tests/ -v（全量） | ✅ 33/33 PASS |
| C5-T5 | pytest tests/test_c5_action_tools.py | ✅ 11/11 PASS |
| C5-T6 | pytest tests/test_c5_confirmation_manager.py | ✅ 11/11 PASS |

### 写操作工具真实执行验证

| # | 场景 | 预期 | 结果 |
|---|------|------|------|
| C5-W1 | 新股加入自选 → `already_exists=False`，DB flush 调用 | `ok=True, already_exists=False` | ✅ |
| C5-W2 | 重复加入自选 → `already_exists=True`，无 DB flush | `ok=True, already_exists=True` | ✅ |
| C5-W3 | savepoint 并发竞争 `IntegrityError` → 不破坏外层事务 | `ok=True, already_exists=True` | ✅ |
| C5-W4 | LLM 初始化失败 → `ok=False, error!=None` | `ok=False` | ✅ |
| C5-W5 | Registry 初始化失败 → `ok=False` | `ok=False` | ✅ |
| C5-W6 | 分析任务成功创建 → `analysis_run` 卡片，`run_id` 有效 | `ok=True, cards[0].type="analysis_run"` | ✅ |
| C5-W7 | 对比 URL 生成 → `compare_link` 卡片，URL 包含 symbol | `cards[0].type="compare_link"` | ✅ |

### ConfirmationManager 生命周期验证

| # | 场景 | 预期 | 结果 |
|---|------|------|------|
| C5-C1 | pending 状态：`is_executable` = True | ✅ | ✅ |
| C5-C2 | expired 状态：`is_executable` = False | ✅ | ✅ |
| C5-C3 | executing 状态：`is_executable` = False（幂等拒绝） | ✅ | ✅ |
| C5-C4 | 超时 10 分钟后：`is_expired` = True | ✅ | ✅ |
| C5-C5 | `make_confirmation()` 包含 id/type/params/status/expires_at 字段 | ✅ | ✅ |

### 前端卡片新增验证

| 卡片类型 | 触发场景 | 状态 |
|---------|---------|------|
| `watchlist_action` (already_exists=False) | 首次加入自选股 confirm 成功 | ✅ 新增渲染 |
| `watchlist_action` (already_exists=True) | 重复加入自选股 | ✅ 新增渲染 |
| `analysis_run` | 分析任务创建成功 | ✅ 新增渲染 |

### Confirmation Card 状态机验证

| 状态 | 触发方式 | UI 表现 | 结果 |
|------|---------|---------|------|
| `pending` | 初始状态 | 确认/取消按钮可用 | ✅ |
| `executing` | 用户点击确认，前端乐观更新 | ⏳ 执行中… | ✅ |
| `executed` | API 返回 success | 按钮消失，卡片变灰 | ✅ |
| `cancelled` | 用户点击取消 | 按钮消失 | ✅ |
| `failed` | API 返回 error | ✗ 执行失败 | ✅ |
| `expired` | 超时后 API 返回 expired | ⏱ 操作已超时 | ✅ |

### 浏览器链路验证（C5-b，手动）

| # | 链路 | 预期 | 验证状态 |
|---|------|------|---------|
| L1 | 自选股卡片「查看自选股」链接 → `/watchlist` | 页面跳转至自选股视图 | 待浏览器验证 |
| L2 | 分析任务卡片「查看报告中心」链接 → `/history` | 页面跳转至历史报告 | 待浏览器验证 |
| L3 | 分析任务卡片「前往分析页」链接 → `/analysis?market=CN&symbol=688146` | 页面打开分析视图 | 待浏览器验证 |
| L4 | 对比卡片「进入对比页」链接 → `/compare?stocks=CN:300750,...` | 页面打开对比视图 | 待浏览器验证 |
| L5 | ConfirmationCard 已执行后确认按钮消失 | isResolved=true → 按钮隐藏 | 待浏览器验证 |
| L6 | 交易意图输入「帮我买入688146」→ 安全拒绝消息 | 无 confirmation，含免责声明 | 待浏览器验证 |

### 安全守卫验证

| 输入 | 预期拦截 | 结果 |
|------|---------|------|
| 帮我买入688146 | ✅ trading 守卫 | ✅ |
| 明天中船特气会涨吗 | ✅ trading 守卫 | ✅ |
| 目标价多少 | ✅ trading 守卫 | ✅ |
| 必涨的股票推荐 | ✅ trading 守卫 | ✅ |
| 中船特气最近为什么涨（非交易意图） | ❌ 不拦截 → anomaly 意图 | ✅ |


---

## Phase C6 Smoke Test（2026-06-18）

### 自动化验证

| # | 测试 | 结果 |
|---|------|------|
| C6-T1 | python -m compileall app -q | ✅ 0 errors |
| C6-T2 | npm run build | ✅ 213 modules，0 errors |
| C6-T3 | alembic current | ✅ d7e3a9b5c2f8 (head，未变) |
| C6-T4 | pytest tests/ -v（全量） | ✅ 72/72 PASS |
| C6-T5 | pytest tests/test_c6_skill_registry.py | ✅ 11/11 PASS |
| C6-T6 | pytest tests/test_c6_financial_skills.py | ✅ 13/13 PASS |
| C6-T7 | pytest tests/test_c6_orchestrator_integration.py | ✅ 10/10 PASS（含 C4/C5 回归 33/33） |

### SkillRegistry 路由验证

| 输入 | 预期 Skill | 结果 |
|------|-----------|------|
| 中船特气最近为什么涨这么多 | StockAnomalySkill | ✅ |
| 帮我重点看中船特气的风险 | RiskFirstSkill | ✅ |
| 688146 最近新闻有什么影响 | NewsCatalystSkill | ✅ |
| 看看我的自选股 | WatchlistReviewSkill | ✅（不走 watchlist_add） |
| 今天哪些行业值得重点研究 | IndustryHotspotSkill | ✅ |
| 解释我最近一份报告 | ReportExplanationSkill | ✅ |
| 把中船特气加入自选 | Action Intent（不走 Skill） | ✅ C5 confirmation |
| 帮我生成688146综合报告 | Action Intent（不走 Skill） | ✅ C5 confirmation |
| 帮我买入688146 | Safety Guard（不走 Skill） | ✅ 拦截 |
| 你好 | Default Handler | ✅ |
...
| # | 优先级层 | 验证 | 结果 |
|---|---------|------|------|
| P1 | Safety Guard > SkillRegistry | "帮我买入" → 安全拒绝，不走 Skill | ✅ |
| P2 | Action Intent > SkillRegistry | "加入自选" → confirmation，不走 Skill | ✅ |
| P3 | SkillRegistry > C4 fallback | "中船特气为什么涨" → StockAnomalySkill，不走旧 _handle_anomaly | ✅ |
| P4 | metadata.skill_name 记录 | OrchestratorResult.metadata["skill_name"] 正确 | ✅ |
| P5 | 无 Skill 匹配时 C4 fallback | "你好" → default_handler | ✅ |

### 安全验证

| 检查项 | 结果 |
|--------|------|
| 6 只 Skills answer 含免责声明 | ✅ 13 个 pytest 用例扫描通过 |
| 6 只 Skills FORBIDDEN_PHRASES 扫描 | ✅ 0 命中（买入/卖出/持有/目标价） |
| 交易意图在 Skill 执行前被拦截 | ✅ safety guard 优先级 > SkillRegistry |
| Skill 工具失败不崩溃 | ✅ 返回 partial SkillResult |
| WatchlistReviewSkill 只说"研究线索"，不说"推荐" | ✅ 代码审计通过 |

---

## Phase C7 Smoke Test（2026-06-18）

### 自动化验证

| # | 测试 | 结果 |
|---|------|------|
| C7-T1 | python -m compileall app -q | ✅ 0 errors |
| C7-T2 | npm run build | ✅ 213 modules，0 errors |
| C7-T3 | alembic current | ✅ d7e3a9b5c2f8 (head，无新 migration) |
| C7-T4 | pytest tests/ -v（全量） | ✅ 137/137 PASS |
| C7-T5 | pytest tests/test_c7_rule_based_planner.py | ✅ 39/39 PASS |
| C7-T6 | pytest tests/test_c7_planner_executor.py | ✅ 14/14 PASS |
| C7-T7 | pytest tests/test_c7_orchestrator_integration.py | ✅ 12/12 PASS |

### RuleBasedPlanner 复合任务检测验证

| 输入 | 预期 intent_type | 结果 |
|------|-----------------|------|
| 688146为什么涨这么多然后重点看风险 | anomaly_then_risk | ✅ |
| 解释这份报告并告诉我主要风险 | report_then_risk | ✅ |
| 看看自选股然后找有没有最近波动大的 | watchlist_scan | ✅ |
| 哪些行业最热然后每个行业挑几个股票看一下 | industry_then_stocks | ✅ |
| 688146为什么涨这么多顺便加自选 | research_then_action | ✅ |
| 比较宁德时代和紫金矿业然后帮我生成报告 | compare_then_report | ✅ |
| 688146 现在多少钱（无连接词/单意图） | None（不是复合任务） | ✅ |

### Orchestrator 分发层级验证（C7 6 层）

| # | 层级 | 验证 | 结果 |
|---|------|------|------|
| P1 | Safety Guard 在 Planner 之前 | "帮我买入" → 安全拒绝，planner_used 不在 metadata | ✅ |
| P2 | 显式 Action Intent 在 Planner 之前 | "加入自选" → confirmation，不走 Planner | ✅ |
| P3 | 复合消息 → Planner | anomaly+risk 复合 → planner_used=True | ✅ |
| P4 | Planner answer 含 _DISCLAIMER | 所有复合任务答案含"仅供研究参考" | ✅ |
| P5 | Planner answer 无 FORBIDDEN_PHRASES | 答案中无买入/卖出/必涨等词语 | ✅ |
| P6 | Planner 失败时 fallthrough 到 SkillRegistry | executor 抛异常 → SkillRegistry 接管 | ✅ |
| P7 | 简单单意图 → SkillRegistry（非 Planner） | metadata.source="skill_registry" | ✅ |

### PlannerExecutor 安全验证

| 检查项 | 结果 |
|--------|------|
| action step 只创建 confirmation，不执行 DB 写操作 | ✅ |
| confirmation status="pending"，type="add_watchlist" | ✅ |
| 禁止词语（买入/卖出/必涨等 8 个）从合成答案中剥离 | ✅ |
| MAX_STEPS=5 强制截断 | ✅ |
| 部分 skill 失败不崩溃，整体 ok=True | ✅ |

---

## Phase C9 — OpenClaw-style Skill Registry Smoke Test（2026-06-18）
...
| `pytest tests/ -q` | ✅ 317/317 PASS |
| `npm run build` | ✅ built in ~940ms |
| `alembic current` | ✅ d7e3a9b5c2f8 (head) — 0 new migrations |

### SkillSpec 文件验证

| 检查 | 结果 |
|------|------|
| 6 个 SkillSpec JSON 文件存在 | ✅ |
| 所有 spec 包含 name/enabled/required_tools/permission_level/safety_rules | ✅ |
| 所有 spec version = c9_v1 | ✅ |
| 所有 spec enabled = true | ✅ |
| 所有 spec permission_level = read_only | ✅ |
| no_trading_advice + must_include_disclaimer 在所有 spec safety_rules 中 | ✅ |

### 技能发现 API

| 检查 | 结果 |
|------|------|
| GET /chat/skills 存在且 path 正确 | ✅ |
| GET /chat/skills 需要 auth（get_current_user 依赖） | ✅ |
| get_skills_list() 返回 6 个技能 | ✅ |
| 返回不含 _source_file / failure_handling 内部字段 | ✅ |
| disabled skill 在返回中 enabled=False | ✅ |

### enabled/available 行为

| 检查 | 结果 |
|------|------|
| disabled skill 不参与 select_skill | ✅ |
| disabled skill select_by_name 返回 None | ✅ |
| unavailable skill 不参与 select_skill | ✅ |
| PlannerExecutor disabled skill step → failed（不崩溃） | ✅ |
| PlannerExecutor unavailable skill step → failed（不崩溃） | ✅ |
| SkillResult.metadata 包含 skill_spec_version | ✅ |

### 安全验证

| 检查 | 结果 |
|------|------|
| 禁用词（买入/卖出/持有/目标价/必涨/稳赚/抄底/追涨）扫描 0 命中 | ✅ |
| SkillSpec 不能声明 sensitive/admin permission_level | ✅ validate_skill_spec 拒绝 |
| C7/C8 回归：137+92 旧测试 0 回归 | ✅ |


---

## Phase C10 Smoke Test（2026-06-18）

### Agent Evaluation + Capability Manifest

| 检查项 | 结果 |
|--------|------|
| `docs/chat_agent_capability_manifest.md` 存在且 > 500 bytes | ✅ |
| `docs/chat_agent_capability_manifest.json` 有效 JSON，tools.count=9，skills.count=6 | ✅ |
| `docs/chat_agent_readiness_checklist.md` 存在 | ✅ |
| `docs/chat_agent_evaluation_report.md` 由脚本自动生成 | ✅ |
| `backend/scripts/evaluate_chat_agent.py` 有 main()/_parse_pytest_output()/_build_report() | ✅ |
| `python evaluate_chat_agent.py --help` 返回 0 | ✅ |
| Golden Tasks 30/30 PASS（A:8 B:6 C:4 D:4 E:3 F:5） | ✅ |
| `test_c10_capability_manifest.py` 42 PASS | ✅ |
| `test_c10_evaluation_script.py` 13 PASS（含 subprocess --help）| ✅ |
| 全量回归：389/389 PASS，0 回归 | ✅ |

---

## Phase C11-c + C12 Smoke Test（2026-06-21）

### C11-c：E2E Acceptance + UX Hardening

| 检查项 | 结果 |
|--------|------|
| `execute_create_analysis_run` scope key 修复（`"scope"` 而非 `"analysis_scope"`） | ✅ |
| `ChatReasoningSteps` 集成进 `ChatMessageList.vue` | ✅ |
| 硬超时不切 demo mode，显示 error card | ✅ |
| 6 语言 `chat_timeout_hard` 不含"演示模式" | ✅ |
| `requested_from = "chat_agent"` 写入 orchestrator 确认 params | ✅ |
| 4 新测试文件（analysis_save/e2e_contract/rag_visibility/timeout_contract）42 tests PASS | ✅ |
| 全量回归：489/489 PASS | ✅ |

### C12：AI Assistant UX Refactor + Real-time Agent Thinking

| 检查项 | 结果 |
|--------|------|
| `ChatCopilotView.vue` 2 列布局，移除 ChatContextPanel | ✅ |
| 发送后 ≤100ms 立即显示 5 个 placeholder 研究步骤 | ✅ |
| API 返回后 placeholder 替换为真实 tool_events | ✅ |
| `ChatQuickActions.vue` 精简为 5 题 + 换一换（3 套轮换） | ✅ |
| 点击快捷问句 → 填入输入框，不自动发送 | ✅ |
| API 失败不切 fallbackMode，显示 error 气泡 | ✅ |
| 首条消息后本地更新 session title preview | ✅ |
| `ChatReasoningSteps` 有 running/pending 步骤时自动展开 | ✅ |
| 标签改为"研究步骤"（`chat_research_steps`，非 `chat_reasoning_steps`） | ✅ |
| 6 语言 11 新 i18n 键（welcome/step/shuffle）| ✅ |
| 4 新测试文件 59 tests PASS | ✅ |
| 全量回归：548/548 PASS，0 回归 | ✅ |
| `npm run build` ✓，0 errors | ✅ |
| 0 新 migration，0 新外部依赖 | ✅ |

### C13-b：Real-time Tool/RAG/Skill Event Streaming（2026-06-22）

| 检查项 | 结果 |
|--------|------|
| `chat_events.py` `safe_emit()` 新增，无副作用，callback 异常静默 | ✅ |
| `ToolRegistry.call()` 新增 `event_callback` 参数，向后兼容 | ✅ |
| `tool_started` 事件在工具执行前推送（payload: tool_name, permission_level）| ✅ |
| `tool_completed` 事件在工具完成后推送（payload: ok, duration_ms, status）| ✅ |
| 未知工具名也推送 `tool_completed(ok=False)` | ✅ |
| `retrieve_context()` 推送 `rag_retrieve_started` / `rag_retrieve_completed` | ✅ |
| `rag_retrieve_completed` 含 `documents_count`, `source_types` | ✅ |
| RAG 失败时推送 `rag_retrieve_completed(ok=False)`，不崩溃 | ✅ |
| 4 个 RAG Skills 在 review 前后推送 `rag_review_started` / `rag_review_completed` | ✅ |
| `rag_review_completed` 含 `overall_confidence`, `approved_for_answer` | ✅ |
| 全部 6 个 Skills 在 run() 开始推送 `skill_started`，结束推送 `skill_completed` | ✅ |
| Skills 内所有 tool_registry.call() 透传 `event_callback` | ✅ |
| `PlannerExecutor` 每步推送 `planner_step_started` / `planner_step_completed` | ✅ |
| Planner 失败步骤推送 `status=failed` | ✅ |
| `chat_streaming.py` Phase 5 去重：已实时发送的 tool 事件不重复 replay | ✅ |
| `rag_retrieve`/`rag_review` 伪 tool 事件跳过 Phase 5 replay | ✅ |
| 前端 `chat.js` dispatch map 新增 8 种事件 handler | ✅ |
| 前端 `ChatCopilotView.vue` 新增 tool_started/completed 动态 running→completed | ✅ |
| 前端 RAG / planner 事件显示 running 状态 | ✅ |
| 禁用词扫描 chat_events.py + chat_streaming.py: 0 hits | ✅ |
| 44 新测试 PASS（5 文件）| ✅ |
| 全量回归：754/754 PASS（12 pre-existing failures 不变）| ✅ |
| `npm run build` ✓，0 errors | ✅ |
| `python -m compileall app -q` ✓，0 errors | ✅ |
| alembic current: d7e3a9b5c2f8（head，无新 migration）| ✅ |
| 0 新外部依赖 | ✅ |

### C13-a：Real-time SSE Streaming（2026-06-21）

| 检查项 | 结果 |
|--------|------|
| `POST /sessions/{id}/messages/stream` 端点注册，返回 `text/event-stream` | ✅ |
| `chat_streaming.py` asyncio.Queue + background Task 模式，db session 安全隔离 | ✅ |
| SSE 事件序列：user_saved → agent_started → placeholder_created → intent_detected → tool_completed → answer_delta → agent_completed | ✅ |
| answer_delta 25字/块，20ms 间隔，重组后等于完整 answer | ✅ |
| keepalive `": keepalive\n\n"` 每 15s 发送，stream-end sentinel `": stream-end\n\n"` | ✅ |
| event_callback 可选参数，失败不影响主流程 | ✅ |
| `sendChatMessageStream()` 使用 fetch+POST+ReadableStream，不用 EventSource | ✅ |
| AbortController per-request，Stop 按钮调用 `.abort()` | ✅ |
| SSE 失败时 fallback 到 `_sendApiSync()`（同步 POST），不说"演示模式" | ✅ |
| `chat_stream_fallback` 键存在于全部 6 语言文件 | ✅ |
| 禁用词扫描 chat_streaming.py：买入/卖出/持有/必涨/稳赚/抄底 = 0 hits | ✅ |
| 4 新测试文件 56 tests PASS | ✅ |
| 全量回归：604/604 PASS，0 回归 | ✅ |
| `npm run build` ✓，0 errors | ✅ |
| `python -m compileall app -q` ✓，0 errors | ✅ |
| 0 新 migration，0 新外部依赖 | ✅ |
